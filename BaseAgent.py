...
class BaseAgent(StdSyncAgent):
    ...
    def counter_all(self, offers, states):
        ...
        for edge_needs, all_partners, issues in [
            ...
            current_step_partners = {# 適正な価格を提示してきたエージェントのリスト
            ...
            for p, x in future_step_offers.items():
                ...
                if step <= awi.n_steps:
                    if future_step_offers[p][QUANTITY] + duplicate_list[
                        step - 1
                    ] <= self.needs_at(step, p):# 将来の必要量に達していなかったらACCEPT
            ...
            for i, partner_ids in enumerate(plist):
                ...
                if offered - needs >= 0:
                    if adjusted_diff < best_plus_diff and needs > 0:
                        best_plus_diff, best_index_plus = adjusted_diff, i# 必要量に達しているものの中で最適なオファー
                else:
                    if adjusted_diff < best_minus_diff and offered > 0:
                        best_minus_diff, best_index_minus = adjusted_diff, i# 必要量に達していないものの中で最適なオファー

            # 必要量に達しているものから最適なものを選択
            ...
            if (
                ...
                # 必要量に達していて、閾値に収まっているオファーたち
                best_indx = best_index_plus
            elif len(plist[best_index_minus]) > 0:
                # 必要量に達しているの中で閾値に収まっているものがなかったら必要量に達していないものから最適を選ぶ
                best_indx = best_index_minus
            else:
                # 閾値に収まっているオファーがなくて、必要量を下回るオファーの組み合わせが存在しない
                has_accept_offer = False

            flag = 0 # まだ受諾処理をしていないことを示す

            # 最適オファー(best_index)の受諾処理
            if has_accept_offer and needs > 0:
                partner_ids = plist[best_indx]# 採用するオファーのパートナー
                others = list(current_step_partners.difference(partner_ids))# 採用しないオファーのパートナー
                ...
                # 受諾しなかったオファーのパートナーに将来の契約でカウンターオファー
                others_s_dict = dict()
                ...
                flag = 1 # accept処理を実行したことを示すフラグ


            # 閾値に収まっているオファーがなくて、必要量を下回るオファーの組み合わせが存在しないとき
            if flag != 1:
                # 今日の交渉でacceptを返していない交渉相手を抜き出す
                other_partners = {
                    _
                    for _ in all_partners
                    if _ not in response.keys() and _ in self.negotiators.keys()
                }
                # 必要量を成功率が高いエージェントを優先して分配
                ...
                # distribute_todays_needsで選ばれなかったパートナーに対し、将来の契約を割り当てる
    ...
    def future_supplie_offer(self, partner_list):
        ...
        for step_offset, partners in [
            ...
            if s + step_offset < n and partners:# 現在のステップ + オフセット < シミュレーションステップ数
                step_needs = int(
                    max(
                        0,
                        (
                            # 1日の生産量 - 現在の在庫入力 - 締結済みの将来の契約量　を３で割る
                            p
                            - awi.current_inventory_input
